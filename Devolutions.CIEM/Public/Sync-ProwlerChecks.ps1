...
.DESCRIPTION
    Cherry-picks commits from the upstream Prowler repository that add or modify
    ...
    After cherry-picking, new checks are automatically converted to PowerShell format.

    Use Get-ProwlerChecks.ps1 to preview available commits first.
...
.PARAMETER CherryPick
    Comma-separated list of commit hashes to cherry-pick.
...
$results = Invoke-CherryPick -Commits $commitsToSync
