BeforeAll {
    ...
    $script:ModuleCommandPattern = '^(Write-CIEMLog|Get-SeverityColor|Get-StatusColor|Get-PSUInstalledEnvironment|Invoke-CIEMJobWithProgress|New-CIEM(?:ErrorContent|InfoContent|SuccessContent)|Connect-CIEM|Set-CIEMSecret|Save-CIEMAzureAuthenticationProfile|Set-CIEMAzureAuthenticationProfileActive|Set-CIEMAzureDiscoverySchedule|Set-CIEMAWSAuthenticationProfile|Update-CIEMProvider|Invoke-CIEMQuery|Get-CIEM(?:AzureArmHierarchy|AzureAuthenticationProfile|AzureDiscoveryRun|AzureDiscoverySchedule|AzureIdentityHierarchy|AttackPath|AttackPathRemediationScript|Check|Config|ConnectorPayloadPreview|ExposureChange|IdentityRiskSignals|IdentityRiskSummary|PAMProgressSummary|PSUJobOutput|Provider|ProviderAuthMethod|ScanEfficiencySummary|ScanRun|Secret))$'
