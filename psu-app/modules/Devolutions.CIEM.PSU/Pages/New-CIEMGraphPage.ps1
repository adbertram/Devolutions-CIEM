function Get-CIEMGraphDataForPage {
    <#
    .SYNOPSIS
        Builds graph data for UI page functions from SQLite-stored collected data.
    .DESCRIPTION
        Discovers the provider-specific module (Devolutions.CIEM.{Provider}) and
        calls its Get-CIEMCollectedData function to read persisted data. Builds
        the identity graph via New-CIEMGraph and returns the exported PSCustomObject
        that the graph query functions expect.

        Caches the result in $Session:GraphState to avoid rebuilding on every
        dynamic element refresh within the same page session.
    #>
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
    ...
    # Check session cache first (avoids rebuilding on every UDDynamic refresh)
    $cacheKey = "GraphData_$ProviderName"
    if ($Session:GraphState -and $Session:GraphState[$cacheKey]) {
    ...
    $collected = & $getDataCmd -ProviderName $ProviderName
    if (-not $collected) { return $null }

    # Build graph in-memory (TenantId provided by provider module)
    $tenantId = if ($collected.TenantId) { $collected.TenantId } else { '' }
    $graph = New-CIEMGraph -EntraData $collected.EntraData -IAMData $collected.IAMData -TenantId $tenantId
    $exported = Export-CIEMGraph -Graph $graph

    # Cache in session state
    if ($Session:GraphState) {
        $Session:GraphState[$cacheKey] = $exported
    }
...
function Show-CIEMGraphIdentitySearch {
    ...
    $gData = Get-CIEMGraphDataForPage -ProviderName $ProviderName
    if (-not $gData) { return }
...
function New-CIEMGraphTabContent {
    ...
    .PARAMETER ProviderName
        The cloud provider name (e.g., 'Azure', 'AWS'). Used for SQLite lookups.
    .PARAMETER IdPrefix
...
function New-CIEMGraphPage {
    ...
    New-UDPage -Name 'Identity Graph' -Url '/ciem/graph' -Content {
        ...
        if ($providersWithData.Count -eq 0) {
        ...
        $providerNames = @($providersWithData.Keys)
