    ...
    process {
        ...
        if ($PSCmdlet.ParameterSetName -eq 'InputObject') {
            foreach ($item in $InputObject) {
                ...
                Invoke-CIEMQuery -Query "INSERT INTO checks (id, provider, service, title, description, risk, severity, remediation_text, remediation_url, related_url, check_script, disabled, permissions, depends_on, data_needs) VALUES (@id, @provider, @service, @title, @description, @risk, @severity, @remediation_text, @remediation_url, @related_url, @check_script, @disabled, @permissions, @depends_on, @data_needs) ON CONFLICT(id) DO UPDATE SET provider=excluded.provider, service=excluded.service, title=excluded.title, description=excluded.description, risk=excluded.risk, severity=excluded.severity, remediation_text=excluded.remediation_text, remediation_url=excluded.remediation_url, related_url=excluded.related_url, check_script=excluded.check_script, disabled=excluded.disabled, permissions=excluded.permissions, depends_on=excluded.depends_on, data_needs=excluded.data_needs" -Parameters @{
