    ...
    process {
        if ($PSCmdlet.ParameterSetName -eq 'InputObject') {
            foreach ($item in $InputObject) {
                ...
                Invoke-CIEMQuery -Query "INSERT OR REPLACE INTO checks (id, provider, service, title, description, risk, severity, remediation_text, remediation_url, related_url, check_script, disabled, permissions, depends_on) VALUES (@id, @provider, @service, @title, @description, @risk, @severity, @remediation_text, @remediation_url, @related_url, @check_script, @disabled, @permissions, @depends_on)" -Parameters @{
            ...
            Invoke-CIEMQuery -Query "INSERT OR REPLACE INTO checks (id, provider, service, title, description, risk, severity, remediation_text, remediation_url, related_url, check_script, disabled, permissions, depends_on) VALUES (@id, @provider, @service, @title, @description, @risk, @severity, @remediation_text, @remediation_url, @related_url, @check_script, @disabled, @permissions, @depends_on)" -Parameters @{
                ...
                disabled = if ($Disabled) { 1 } else { 0 }; permissions = $Permissions; depends_on = $DependsOn
            } -AsNonQuery | Out-Null
        }
    }
}
