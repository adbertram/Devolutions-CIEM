function Invoke-CIEMScan {
    ...
    param(
        ...
        [Parameter()]
        [ValidateRange(1, 100)]
        [int]$ThrottleLimit = $(if ((Get-CIEMConfig).scan.throttleLimit) { (Get-CIEMConfig).scan.throttleLimit } else { 10 }),
    ...
    try {
        ...
        foreach ($providerName in $Provider) {
            ...
            $authContext    = Get-CIEMRuntimeAuth -Provider $providerName
            ...
            $checkScriptsPath = Join-Path -Path $script:ChecksRoot -ChildPath "Checks/$providerName"
