function Save-CIEMAzureSecurityPrincipal {
    ...
    process {
        ...
        foreach ($item in $items) {
            ...
            if ($item) { $p = @{ id=$item.Id; provider_id=$item.ProviderId; type=$item.Type; display_name=$item.DisplayName; enabled=if($null -eq $item.Enabled){$null}elseif($item.Enabled){1}else{0}; category=$item.Category; principal_type=$item.PrincipalType; upn=$item.UserPrincipalName; user_type=$item.UserType; app_id=$item.AppId; sp_type=$item.ServicePrincipalType; now=$now } }
            else { $p = @{ id=$Id; provider_id=$ProviderId; type=$Type; display_name=$DisplayName; enabled=if($null -eq $Enabled){$null}elseif($Enabled){1}else{0}; category=$Category; principal_type=$PrincipalType; upn=$UserPrincipalName; user_type=$UserType; app_id=$AppId; sp_type=$ServicePrincipalType; now=$now } }
            Invoke-CIEMQuery -Query "INSERT OR REPLACE INTO azure_security_principals (id, provider_id, type, display_name, enabled, category, principal_type, user_principal_name, user_type, app_id, service_principal_type, collected_at) VALUES (@id, @provider_id, @type, @display_name, @enabled, @category, @principal_type, @upn, @user_type, @app_id, @sp_type, @now)" -Parameters $p -AsNonQuery | Out-Null
        }
    }
}
