...
Describe 'ConvertTo-CIEMGraphMermaid' {
    ...
    BeforeAll {
        ...
        $script:testGraphData = InModuleScope Devolutions.CIEM.Identities {
            $graph = [CIEMGraph]::new()
    ...
    Context 'Node deduplication' {
        BeforeAll {
            ...
            $script:dedupGraphData = InModuleScope Devolutions.CIEM.Identities {
                $graph = [CIEMGraph]::new()
                $graph.TenantId = 'test-tenant'
    ...
    Context 'Label sanitization' {
        BeforeAll {
            $script:specialCharData = InModuleScope Devolutions.CIEM.Identities {
                $graph = [CIEMGraph]::new()
                $graph.TenantId = 'test-tenant'
