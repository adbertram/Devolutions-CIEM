...
Describe 'New-CIEMGraph' {
    ...
    Context 'Node creation' {
        It 'Creates user nodes from Entra data' {
            InModuleScope Devolutions.CIEM.Identities -Parameters @{ EntraData = $script:mockEntraData } {
                param($EntraData)
        ...
        It 'Creates role assignment and definition nodes from IAM data' {
            InModuleScope Devolutions.CIEM.Identities -Parameters @{ EntraData = $script:mockEntraData; IAMData = $script:mockIAMData } {
