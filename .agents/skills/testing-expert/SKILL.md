...
| Clipboard assertions on local PSU | Read `navigator.clipboard` directly on `http://192.168.86.36:5001` | Install a test-only clipboard shim with `page.addInitScript()` before navigation | Chromium does not expose Clipboard API on private-IP HTTP origins |
