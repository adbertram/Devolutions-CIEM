...
<critical_rule>
**DEBUG BEFORE PUBLISH:** Publishing is slow. ALWAYS test fixes via `Invoke-TestCommand` BEFORE publishing a new module version.

Workflow: Identify error → Write fix locally → Test fix via Invoke-TestCommand → Confirm fix works → THEN publish
</critical_rule>
...
## Step 9: Test fix via Invoke-TestCommand BEFORE publishing (MANDATORY)
...
Invoke-TestCommand -ScriptBlock { Get-CIEMProvider -Name Azure } -Destination local

# Test on local PSU app (runs inside PSU's PowerShell environment)
Invoke-TestCommand -ScriptBlock {
