...
6. Run `scripts/send-report.sh reports/YYYY-MM-DD` to send everything to Slack
</quick_start>
...
```bash
# First page — open browser and navigate
playwright-cli open http://localhost:5001/ciem/ciem/<page>
# CRITICAL: Wait for PSU page to fully render (UDDynamic content, API calls, graphs)
sleep 5
# Take a snapshot first to confirm content is loaded (not skeletons/spinners)
playwright-cli snapshot
# Only then capture the screenshot
playwright-cli screenshot --filename "<report-dir>/<page-name>.png" --full-page

# Subsequent pages — navigate and wait
...
Run the send script from the skill directory:
...
The script:
1. Uploads all `*.png` files to the Slack channel (screenshots appear first)
