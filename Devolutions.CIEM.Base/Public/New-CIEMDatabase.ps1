function New-CIEMDatabase {
    ...
    .DESCRIPTION
        ...
        The database is stored outside the PSU Repository directory to avoid
        triggering PSU's file system watcher.
    .PARAMETER Path
    ...
    if (-not $Path) {
        if ($Repository) {
            # Running inside PSU — $Repository is a PSU built-in variable
            $basePath = Split-Path $Repository -Parent
        } elseif ($script:ModuleRoot) {
            # Local dev — module is at <project>/Devolutions.CIEM.Base/
            $projectRoot = Split-Path $script:ModuleRoot -Parent
            $basePath = Join-Path $projectRoot 'local-psu'
        } else {
            $basePath = $PWD.Path
        }
